...
function gh_parse_git_status {
    ...
    if [ -z "$1" ]; then
        gh_log "gh_parse_git_status: Invalid usage. A parameter matching status type is required."
        return 1
    fi
    ...
    case $1 in
        all)
            ...
            _pgs_onremote=
            [ "$showremotestate" = "y" ] && [ -n "$( git branch -r --list */$( gh_parse_git_branch ) )" ] && _pgs_onremote=true
        ...
        onremote)
            [ -n "$( git branch -r --list */$( gh_parse_git_branch ) )" ]
            return $?
            ;;

        tracking)
            git config branch.$( gh_parse_git_branch ).remote >/dev/null
            return $?
            ;;

        *)
            gh_log "gh_parse_git_status: Invalid parameter given  <$1>"
            return 1
            ;;
    esac
